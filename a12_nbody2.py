...
init_p = deepcopy(positions)
init_v = deepcopy(velocities)
...
def period(axis):
    '''Calculate the period for a single axis'''
    global steps, positions, velocities
    steps = 0
    positions = deepcopy(init_p)
    velocities = deepcopy(init_v)
    while True:
        step()
        if ([x[axis] for x in positions] == [x[axis] for x in init_p] and 
            [x[axis] for x in velocities] == [x[axis] for x in init_v]):
            return steps
...
x_period = period(0)
print(f'x_period = {x_period}')
y_period = period(1)
print(f'y_period = {y_period}')
z_period = period(2)
print(f'z_period = {z_period}')

print(f'Whole cycle repeats at {lcm(lcm(x_period,y_period), z_period)}')
